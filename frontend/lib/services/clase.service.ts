...
export const getSalones = async (
  ...
  return httpClient.get<PaginatedResponse<Salon>>(
    `/api/salones/?${params.toString()}`
  );
};
...
export const getClases = async (
  ...
  return httpClient.get<PaginatedResponse<Clase>>(
...
export const getInscripciones = async (
  ...
  return httpClient.get<PaginatedResponse<InscripcionClase>>(
    `/api/inscripciones-clase/?${params.toString()}`
  );
};
...
export const updateInscripcion = async (
  ...
  return httpClient.put<InscripcionClase>(
...
export const getClasesDisponibles = async (filtros?: {
  ...
  try {
    ...
    if (filtros?.disciplina)
      params.append("disciplina", filtros.disciplina.toString());
    if (filtros?.instructor)
      params.append("instructor", filtros.instructor.toString());
    ...
    return clases.filter(
      (clase) => !clase.esta_llena && (clase.cupos_disponibles ?? 0) > 0
    );
  } catch (error) {
