    ...
    def validate(self, data):
        ...
        if inscripcion and clase:
            if self.instance is None:  # Solo en creación
                if AsistenciaClase.objects.filter(
...
class RegistrarAsistenciaSerializer(serializers.Serializer):
    ...
    estado = serializers.ChoiceField(
        choices=EstadoAsistencia.choices, default=EstadoAsistencia.PRESENTE
    )
    ...
    def create(self, validated_data):
        ...
        try:
            # Crear instancia sin guardar primero para poder validar
            ...
            # El método save() del modelo llama a full_clean() automáticamente
            asistencia.save()
            return asistencia
            ...
            if hasattr(e, "message_dict"):
                ...
                # Si tiene mensajes múltiples, tomar el primero
            ...
            # Capturar cualquier otro error y convertirlo a ValidationError de DRF
            import traceback

            print(
                f"Error inesperado al crear asistencia: {type(e).__name__}: {str(e)}",
                file=__import__("sys").stderr,
            )
            print(traceback.format_exc(), file=__import__("sys").stderr)
            raise serializers.ValidationError(
                {"non_field_errors": [f"Error al registrar asistencia: {str(e)}"]}
            )
...
class RegistrarAsistenciasMasivasSerializer(serializers.Serializer):
    ...
    asistencias = serializers.ListField(child=serializers.DictField(), min_length=1)
    ...
    def validate_asistencias(self, value):
        ...
        for item in value:
            if "inscripcion_id" not in item:
                raise serializers.ValidationError(
                    "Falta inscripcion_id en algún registro"
                )
    ...
    def create(self, validated_data):
        ...
        for data in asistencias_data:
            try:
                inscripcion = InscripcionClase.objects.select_related("cliente").get(
                    id=data["inscripcion_id"], clase=clase
                )

                # Verificar que no existe ya
                if AsistenciaClase.objects.filter(inscripcion=inscripcion).exists():
                    errores.append(
                        f"Cliente {inscripcion.cliente.nombre_completo}: Ya registrado"
                    )
                    continue
