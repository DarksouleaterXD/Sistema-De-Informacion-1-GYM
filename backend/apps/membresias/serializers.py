...
class InscripcionMembresiaSerializer(serializers.ModelSerializer):
    ...
    metodo_de_pago_display = serializers.CharField(
        source="get_metodo_de_pago_display", read_only=True
    )
...
class MembresiaPromocionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "membresia", "promocion", "promocion_info", "created_at"]
...
class MembresiaSerializer(serializers.ModelSerializer):
    ...
    inscripcion_info = InscripcionMembresiaSerializer(
        source="inscripcion", read_only=True
    )
    ...
    def validate(self, data):
        ...
        # Validar promociones si se proporcionan
    ...
    def update(self, instance, validated_data):
        """Actualizar membresía incluyendo promociones"""
        promociones_ids = validated_data.pop("promociones", None)

        # Actualizar campos normales
        instance = super().update(instance, validated_data)

        # Actualizar promociones si se proporcionan
        if promociones_ids is not None:
            instance.promociones.set(promociones_ids)
...
class MembresiaListSerializer(serializers.ModelSerializer):
    ...
    cliente_nombre = serializers.CharField(
        source="inscripcion.cliente.nombre_completo", read_only=True
    )
    cliente_ci = serializers.CharField(source="inscripcion.cliente.ci", read_only=True)
    plan_nombre = serializers.CharField(source="plan.nombre", read_only=True)
    monto = serializers.DecimalField(
        source="inscripcion.monto", max_digits=10, decimal_places=2, read_only=True
    )
    estado_display = serializers.CharField(source="get_estado_display", read_only=True)
    dias_restantes = serializers.ReadOnlyField()
    esta_activa = serializers.ReadOnlyField()
...
class MembresiaCreateSerializer(serializers.Serializer):
    ...
    metodo_de_pago = serializers.ChoiceField(
        choices=[choice[0] for choice in METODOS_PAGO]
    )
    ...
    estado = serializers.ChoiceField(
        choices=[choice[0] for choice in ESTADOS_MEMBRESIA]
    )
