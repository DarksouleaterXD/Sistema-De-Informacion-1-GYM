    ...
    def validate_nombre(self, value):
        ...
        if queryset.exists():
            raise serializers.ValidationError(
                f"Ya existe un salón con el nombre '{value}'"
            )
...
class ClaseSerializer(serializers.ModelSerializer):
    disciplina_nombre = serializers.CharField(
        source="disciplina.nombre", read_only=True
    )
    ...
    def validate(self, data):
        ...
        if salon and cupo_maximo:
            # Obtener el objeto Salon si es un ID
            if isinstance(salon, int):
                try:
                    salon_obj = Salon.objects.get(pk=salon)
                except Salon.DoesNotExist:
                    raise serializers.ValidationError(
                        {"salon": "El salón especificado no existe"}
                    )
            else:
                salon_obj = salon

            if cupo_maximo > salon_obj.capacidad:
                raise serializers.ValidationError(
                    {
                        "cupo_maximo": f"El cupo máximo no puede exceder la capacidad del salón ({salon_obj.capacidad})"
                    }
                )
        ...
        if fecha and salon and hora_inicio and hora_fin:
            # Asegurar que salon sea un ID (número)
            salon_id = (
                salon
                if isinstance(salon, int)
                else (salon.id if hasattr(salon, "id") else salon)
            )
        ...
        if fecha and instructor and hora_inicio and hora_fin:
            # Asegurar que instructor sea un ID (número)
            instructor_id = (
                instructor
                if isinstance(instructor, int)
                else (instructor.id if hasattr(instructor, "id") else instructor)
            )
            ...
            clases_instructor = Clase.objects.filter(
                ...
                fecha=fecha,
                estado__in=[CLASE_PROGRAMADA, CLASE_EN_CURSO],
            ).filter(Q(hora_inicio__lt=hora_fin, hora_fin__gt=hora_inicio))
...
class InscripcionClaseSerializer(serializers.ModelSerializer):
    ...
    disciplina_nombre = serializers.CharField(
        source="clase.disciplina.nombre", read_only=True
    )
    ...
    cupos_disponibles = serializers.IntegerField(
        source="clase.cupos_disponibles", read_only=True
    )
    estado_display = serializers.CharField(source="get_estado_display", read_only=True)
    ...
    def get_clase_info(self, obj):
        return (
            f"{obj.clase.disciplina.nombre} - {obj.clase.fecha} {obj.clase.hora_inicio}"
        )
        ...
        if not self.instance:
            ...
            if cliente:
                ...
                tiene_membresia_activa = Membresia.objects.filter(
                    inscripcion__cliente=cliente, estado=ESTADO_ACTIVO
                ).exists()
            ...
            if clase and cliente:
                ya_inscrito = (
                    InscripcionClase.objects.filter(clase=clase, cliente=cliente)
                ...
                if ya_inscrito:
                    raise serializers.ValidationError(
                        {
                            "non_field_errors": [
                                "El cliente ya está inscrito en esta clase."
                            ]
                        }
                    )
